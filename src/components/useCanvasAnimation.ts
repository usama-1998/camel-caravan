...
export const useCanvasAnimation = (
    ...
    useEffect(() => {
        ...
        /* ── Pattern constants ── */
        const TILE = 110;
        const GLOW_R = options.glowRadius || 220;
        ...
        function drawBridges(cx: number, cy: number, outerR: number, alpha: number, lw: number, tile: number) {
            if (alpha < 0.005) return;
            ctx!.strokeStyle = `rgba(${STAR_COLOR},${alpha * 0.55})`;
        ...
        function drawSecondaryPattern(cx: number, cy: number, size: number, alpha: number, lw: number) {
            if (alpha < 0.005) return;
            ctx!.strokeStyle = `rgba(${STAR_COLOR},${alpha * 0.35})`;
        ...
        function drawRadials(cx: number, cy: number, innerR: number, outerR: number, alpha: number, lw: number) {
            if (alpha < 0.005) return;
            ctx!.strokeStyle = `rgba(${STAR_COLOR},${alpha * 0.2})`;
        ...
        function animate(now: number) {
            ...
            // Background gradient is handled by CSS in the parent section
            ...
            mg.addColorStop(0, `rgba(${STAR_COLOR},0.055)`);
            mg.addColorStop(0.4, `rgba(${STAR_COLOR},0.02)`);
            mg.addColorStop(1, `rgba(${STAR_COLOR},0)`);
            ctx!.fillStyle = mg;
            ...
            for (const p of particles) {
                ...
                ctx!.fillStyle = `rgba(${STAR_COLOR},${po})`;
                ctx!.beginPath();
            ...
            /* ── Vignette ── */
            // CTA specific: simpler vignette or handled by parent CSS? 
            // Keeping it simple for CTA as per request "looks alive like the hero section"
            // But maybe lighter vignette for CTA
            const vig = ctx!.createRadialGradient(W / 2, H / 2, W * 0.22, W / 2, H / 2, Math.max(W, H) * 0.85);
            vig.addColorStop(0, 'rgba(0,0,0,0)');
            vig.addColorStop(1, 'rgba(0,0,0,0.15)'); // Lighter vignette for CTA
            ctx!.fillStyle = vig;
            ctx!.fillRect(0, 0, W, H);
